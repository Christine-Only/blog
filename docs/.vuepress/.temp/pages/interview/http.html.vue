...
<h2 id="http-2" tabindex="-1"><a class="header-anchor" href="#http-2"><span>HTTP/2</span></a></h2>
<p>HTTP/2 相比于 HTTP/1，可以说是大幅度提高了网页的性能。</p>
<p>在 HTTP/1 中，为了性能考虑，我们会引入雪碧图、将小图内联、使用多个域名等等的方式。这一切都是因为<strong>浏览器限制了同一个域名下的请求数量（Chrome 下一般是限制六个连接）</strong>，当页面中需要请求很多资源的时候，<strong>队头阻塞</strong>（Head of line blocking）会导致在达到最大请求数量时，剩余的资源需要等待其他资源请求完成后才能发起请求。</p>
<p>在 HTTP/2 中引入了<strong>多路复用</strong>的技术，这个技术可以只通过一个 <strong>TCP</strong> 连接就可以传输所有的请求数据。多路复用很好的解决了浏览器限制同一个域名下的请求数量的问题，同时也间接更容易实现全速传输，毕竟新开一个 TCP 连接都需要慢慢提升传输速度。</p>
<h3 id="二进制传输" tabindex="-1"><a class="header-anchor" href="#二进制传输"><span>二进制传输</span></a></h3>
<p>在之前的 HTTP 版本中，我们是通过<strong>文本的方式传输数据</strong>。在 HTTP/2 中引入了新的编码机制，所有传输的数据都会被分割，并采用二进制格式编码。</p>
<h3 id="多路复用" tabindex="-1"><a class="header-anchor" href="#多路复用"><span>多路复用</span></a></h3>
<p>在 HTTP/2 中，有两个非常重要的概念，分别是<strong>帧（frame）<strong>和</strong>流（stream）</strong>。</p>
...
<h3 id="请求优先级" tabindex="-1"><a class="header-anchor" href="#请求优先级"><span>请求优先级</span></a></h3>
...
<h3 id="首部压缩" tabindex="-1"><a class="header-anchor" href="#首部压缩"><span>首部压缩</span></a></h3>
...
<h3 id="服务器推送" tabindex="-1"><a class="header-anchor" href="#服务器推送"><span>服务器推送</span></a></h3>
<p>HTTP2.0新增的一个强大的新功能，就是服务器可以对一个客户端请求发送多个响应。服务器向客户端推送资源无需客户端明确的请求。</p>
<p>在HTTP2.0中，服务端根据客户端的请求，提前返回多个响应，推送额外的资源给客户端。比如，客户端请求stream 1(/page.html)。服务端在返回stream 1的消息的同时推送了stream 2(/script.js)和stream 4(/style.css)</p>
<h2 id="http-3" tabindex="-1"><a class="header-anchor" href="#http-3"><span>HTTP/3</span></a></h2>
...
<p>QUIC 虽然基于 UDP，但是在原本的基础上新增了很多功能，比如多路复用、0-RTT、使用 TLS1.3 加密、流量控制、有序交付、重传等等功能。</p>
<h3 id="多路复用-1" tabindex="-1"><a class="header-anchor" href="#多路复用-1"><span>多路复用</span></a></h3>
...
<h3 id="_0-rtt" tabindex="-1"><a class="header-anchor" href="#_0-rtt"><span>0-RTT</span></a></h3>
<p>通过使用类似 TCP 快速打开的技术，缓存当前会话的上下文，在下次恢复会话的时候，只需要将之前的缓存传递给服务端验证通过就可以进行传输了。</p>
<h3 id="纠错机制" tabindex="-1"><a class="header-anchor" href="#纠错机制"><span>纠错机制</span></a></h3>
...
<h2 id="状态码" tabindex="-1"><a class="header-anchor" href="#状态码"><span>状态码</span></a></h2>
<ul>
<li>1xx 消息响应</li>
<li>2xx 成功响应</li>
<li>3xx 重定向消息</li>
...
<li>403：告诉客户端禁止访问该站点或者资源，如在外网环境下，然后访问只有内网 IP 才能访问的时候则返回</li>
<li>404：服务器找不到资源时，或者服务器拒绝请求又不想说明理由时</li>
<li>503：服务器停机维护时，主动用 503 响应请求或 nginx 设置限速，超过限速，会返回 503</li>
<li>504：网关超时</li>
</ul>
<h3 id="常见的状态码" tabindex="-1"><a class="header-anchor" href="#常见的状态码"><span>常见的状态码</span></a></h3>
<p>下面是常见的 HTTP 状态码：</p>
<ul>
<li>200 - 请求成功</li>
<li>204 - 请求处理成功，但没有任何资源可以返回给客户端，一般在只需要从客户端往服务器发送信息，而对客户端不需要发送新信息内容的情况下使用。</li>
<li>301 - 资源（网页等）被永久转移到其它URL</li>
<li>302 - 资源（网页等）被临时转移到其它URL</li>
<li>400 - 服务器端无法理解客户端发送的请求，请求报文中可能存在语法错误。</li>
<li>401- 请求要求用户的身份认证，一般是指未被授权，请进行相关的身份验证，比如登录</li>
<li>404 - 请求的资源（网页等）不存在，常常是代码错误，路径错误，网址输入错误或者是网络不佳，网络拥堵等等造成的</li>
<li>418 - 服务器检测请求头，检查出你的操作疑似爬虫，总而拒绝，返回418</li>
